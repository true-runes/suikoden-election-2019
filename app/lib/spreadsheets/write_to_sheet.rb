  ...
  class WriteToSheet
    ...
    def gensui_oshi_serifu
      ...
      TargetTweet.gensui_oshi_serifu.order('tweeted_at asc').each_with_index do |tweet, i|
        ...
        target_sheet[start_row_number + i, start_column_number + 6] =
          %(=HYPERLINK("#{tweet.media_url_https_01}",IMAGE("#{tweet.media_url_https_01}",1)))
        target_sheet[start_row_number + i, start_column_number + 7] =
          %(=HYPERLINK("#{tweet.media_url_https_02}",IMAGE("#{tweet.media_url_https_02}",1)))
        target_sheet[start_row_number + i, start_column_number + 8] =
          %(=HYPERLINK("#{tweet.media_url_https_03}",IMAGE("#{tweet.media_url_https_03}",1)))
        target_sheet[start_row_number + i, start_column_number + 9] =
          %(=HYPERLINK("#{tweet.media_url_https_04}",IMAGE("#{tweet.media_url_https_04}",1)))
      end
